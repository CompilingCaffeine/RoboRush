...
The browser build carries one Wavedash leaderboard: **fastest victory**, ascending, in
milliseconds. A losing run is never posted to it. That is the whole of the rule, and it is
enforced in [leaderboard.gd](autoload/leaderboard.gd) rather than at each place a run can end,
because on an ascending board a mistake is permanent — a four-minute death on floor two beats
every genuine finish there will ever be, and nothing correct afterwards displaces it.

The board's name carries the campaign's content version, so each version gets its own board. A
content bump means a seed no longer builds the run it used to — that is what the version exists
to say — and a route thirty seconds shorter after a floor is rebalanced is not a better run than
the one before it. Ranking them together would quietly make the board a record of which build
somebody played; separating them is what a speedrun category is, and the old board keeps its
times rather than being overwritten by a game that has moved on.

There is no pending-submissions file. `BestRunStats.fastest_victory` is already saved with the
rest of the player's progress, so **the record is the queue**: a victory won during an outage is
re-offered at the start of every session until the board accepts it, and there is no second copy
of it to fall out of step with the first. That is also what makes a fresh install on a second
machine correct — the board, not the device, is the authority on whether a time was ever posted.

The platform sits behind [`LeaderboardBackend`](scripts/cloud/leaderboard_backend.gd), never
`WavedashSDK` directly, for the same reason cloud saving does: everything worth testing here is a
failure, and failures cannot be ordered from a live service. `tests/test_leaderboard.gd` drives a
fake board through outages, bounded retries, a stale device that must not demote a better time,
and three victories arriving inside one post.

A won run shows where it left the player — `GLOBAL RANK  #7` — under the statistics on the run
summary. It is read back from the board rather than taken from the rank the post returns, because
a post only happens when the time is a personal best: a player winning a second time slightly
slower would otherwise see nothing, and "you are still seventh" is the answer they are looking at
that line to get. While the time is still going up the line says so, and if it never lands it says
that instead — the record is saved either way.

The menu entry and the rank line are offered in the browser only. Off the web there is no host
page and no account, so a desktop player would be told about a feature of a different build — the
same rule that takes QUIT out of the browser build, applied the other way round. In a browser both
appear whether or not anyone is signed in, because *there* the answer is something the player can
act on.
